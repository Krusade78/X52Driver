		...
		if(nEje != 0)
		{
			...
			else
			{
				if( *((USHORT*)&nuevoHID.Ejes[idx * 2])!= *((USHORT*)&viejo->Ejes[idx * 2]) )
				{
					...
					{
						if(invertido)
							GenerarAccionRaton(devExt, nEje - 9, ( ((topes[idx] / 2) - (*((UINT16*)&entrada->Ejes[idx * 2]))) *sRaton) / (topes[idx] / 2) );
						else
							GenerarAccionRaton(devExt, nEje - 9, - (( ((topes[idx] / 2) - (*((UINT16*)&entrada->Ejes[idx * 2]))) *sRaton) / (topes[idx] / 2)) );
					...
					{
						if(invertido)
							GenerarAccionRaton(devExt, nEje - 9, - (((*((UINT16*)&entrada->Ejes[idx * 2])-(topes[idx] / 2)) * sRaton) / (topes[idx] / 2)) );
						else
							GenerarAccionRaton(devExt, nEje - 9, ((*((UINT16*)&entrada->Ejes[idx * 2]) - (topes[idx] / 2))* sRaton) / (topes[idx] / 2) );
					}
				}
